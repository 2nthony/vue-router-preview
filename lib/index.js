function installRouterPreview(
  ...
  const offsetPreview = ({ target: el }, _scale) => {
    if (
      el.offsetLeft + el.offsetWidth + safetyPadding >=
        window.innerWidth - window.innerWidth * _scale &&
      el.offsetTop + el.offsetHeight - safetyPadding <
  ...
  const Link = {
    ...
    methods: {
      onMouseEnter(MouseEvent) {
        ...
        clearTimeout(showTimer)
        offsetPreview(MouseEvent, this.scale)
        cleanPreview()
